...
namespace Foxoft.Ci {
  ...
  public class CiMacro : CiSymbol {
    ...
    public CiMacro(CodePosition p, string name) : base(p, name) {
    }
  }
  ...
  public partial class CiParser : CiLexer {
    ...
    CiMacro ParseMacro() {
      string macroName = ParseId();
      CiMacro macro = new CiMacro(Here(), macroName);
      ...
      if (See(CiToken.Id)) {
        do {
          ...
          if (paramz.Contains(name)) {
            throw new ParseException(Here(), "Duplicate macro parameter {0}", name);
          }
          paramz.Add(name);
        }
